...
function pendulum({
    tailColor = [0, 0, 1],
    massColor = [0, 0, 0],
    init = null // angle one, angle two, speed 1, speed 2
} = {}) {
...
function draw_phase(i, canvas, ctx, a1, a2, p1, p2){
    ctx.strokeStyle = "red";
    ctx.beginPath();
    ctx.arc(canvas.width/2.+(Math.atan2(Math.sin(a2), Math.cos(a2))*(canvas.width/Math.PI)), canvas.height/2.-p2*(canvas.width/Math.PI), 1, 0, 2*Math.PI);
    ctx.stroke();
} // the atan2 is used to normalize the angle to [-pi, pi]

function draw_axes(canvas, ctx){
    ctx.font = "20px Arial";
    ctx.fillText("\u03C9", 5+canvas.width/2., 20);
    ctx.fillText("\u03B8", canvas.width-15, canvas.height/2.+20);
    ctx.beginPath();
...
(function() {
    let state1 = [new pendulum()];
    let state2 = [new pendulum()];
    let state3 = [new pendulum()];

    let params = new URL(document.location);
    let useWebGL = params.searchParams.get("webgl") !== '0';
    let canvas_1_2d = document.getElementById('double_pendulum_canvas_1_2d');
    let canvas_1_3d = document.getElementById('double_pendulum_canvas_1_3d');
    let canvas_2_2d = document.getElementById('double_pendulum_canvas_2_2d');
    let canvas_2_3d = document.getElementById('double_pendulum_canvas_2_3d');
    let canvas_3_2d = document.getElementById('double_pendulum_canvas_3_2d');
    let canvas_3_3d = document.getElementById('double_pendulum_canvas_3_3d');
    
    // Phase Diagram canvas
    let canvas_1_DP = document.getElementById('double_pendulum_DP1');
    let canvas_2_DP = document.getElementById('double_pendulum_DP2');
    let canvas_3_DP = document.getElementById('double_pendulum_DP3');
    let DP1 = canvas_1_DP.getContext('2d');
    let DP2 = canvas_2_DP.getContext('2d');
    let DP3 = canvas_3_DP.getContext('2d');
    draw_axes(canvas_1_DP, DP1);
    draw_axes(canvas_2_DP, DP2);
    draw_axes(canvas_3_DP, DP3);
    // up to here
    ...
    let mode;
    let running = true;
    let gl1 = useWebGL ? canvas_1_3d.getContext('webgl') : null;
    let gl2 = useWebGL ? canvas_2_3d.getContext('webgl') : null;
    let gl3 = useWebGL ? canvas_3_3d.getContext('webgl') : null;

    let ctx1 = canvas_1_2d.getContext('2d');
    let ctx2 = canvas_2_2d.getContext('2d');
    let ctx3 = canvas_3_2d.getContext('2d');
    ...
    if (!gl1) {
        ...
        canvas_1_3d.style.display = 'none';
        canvas_2_3d.style.display = 'none';
        canvas_3_3d.style.display = 'none';
    } else {
        renderer1 = new glRenderer(gl1, tailMax);
        renderer2 = new glRenderer(gl2, tailMax);
        renderer3 = new glRenderer(gl3, tailMax);
        mode = '3d';
        canvas1 = canvas_1_3d;
        canvas2 = canvas_2_3d;
        canvas3 = canvas_3_3d;

        canvas_1_2d.style.display = 'none';
        canvas_2_2d.style.display = 'none';
        canvas_3_2d.style.display = 'none';
    }
    ...
    function toggleMode() {
        switch (mode) {
            case '2d':
                ...
                canvas_1_3d.style.display = 'block';
                canvas_2_3d.style.display = 'block';
                canvas_3_3d.style.display = 'block';
            ...
            case '3d':
                ...
                canvas_1_2d.style.display = 'block';
                canvas_2_2d.style.display = 'block';
                canvas_3_2d.style.display = 'block';
    ...
    window.addEventListener('keypress', function(e) {
        switch (e.key) {
            ...
            case "r":
                running = false;
                state1[0] = new pendulum();
                state2[0] = new pendulum();
                state3[0] = new pendulum();
                DP1.clearRect(0, 0, canvas_1_DP.width, canvas_1_DP.height);
                DP2.clearRect(0, 0, canvas_2_DP.width, canvas_2_DP.height);
                DP3.clearRect(0, 0, canvas_3_DP.width, canvas_3_DP.height);
                draw_axes(canvas_1_DP, DP1);
                draw_axes(canvas_2_DP, DP2);
                draw_axes(canvas_3_DP, DP3);
                running = true;
    ...
    function cb(t) {
        ...
        if (running){
            for (let i = 0; i < state1.length; i++)
                state1[i].step(dt / 1000.0);
            for (let i = 0; i < state2.length; i++)
                state2[i].step(dt / 1000.0);
            for (let i = 0; i < state3.length; i++)
                state3[i].step(dt / 1000.0);
        ...
        if (mode === '3d') {
            ...
            renderer1.renderAll(state1);
            renderer2.renderAll(state2);
            renderer3.renderAll(state3);
            ...
            for (let i = 0; i < state1.length; i++)
                state1[i].draw2d(ctx1);
            for (let i = 0; i < state2.length; i++)
                state2[i].draw2d(ctx2);
            for (let i = 0; i < state3.length; i++)
                state3[i].draw2d(ctx3);
        }

        draw_phase(1, canvas_1_DP, DP1, state1[0].state()[0], state1[0].state()[1], state1[0].state()[2], state1[0].state()[3]);
        draw_phase(2, canvas_2_DP, DP2, state2[0].state()[0], state2[0].state()[1], state2[0].state()[2], state2[0].state()[3]);
        draw_phase(3, canvas_3_DP, DP3, state3[0].state()[0], state3[0].state()[1], state3[0].state()[2], state3[0].state()[3]);
    }
        last = t;
